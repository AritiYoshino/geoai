...
Run ID：exp2_20260513_153301

生成时间：2026-05-13T15:33:38
...
任务展开规模：12 个任务单元；本次 trace 数：36；成功 25，失败 11。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| base_agent | 0.160 | 3.488 | 8 | 0.333 | 0.000 | 0.000 | 0.000 | 0.667 | 0.778 | 0.333 | 1.000 | 8 |
| rag_agent | 0.160 | 3.037 | 3 | 0.750 | 0.000 | 0.417 | 0.000 | 0.000 | 0.907 | 0.750 | 1.000 | 3 |
| ace_agent | 0.160 | 2.763 | 0 | 1.000 | 0.250 | 0.417 | 0.000 | 0.000 | 1.000 | 1.000 | 1.000 | 0 |
...
实验二最终 batch 的成功率为 ACE=1.000、RAG=1.000、BASE=0.333；可结合经验复用率和新经验数量解释 ACE 连续学习相对静态检索与无记忆基线的优势。

基于连续任务学习实验（exp2）结果，主要发现如下：

**主要发现：** ACE智能体在所有批次中均实现了100%的任务成功率，而基础智能体（33.3%）和RAG智能体（75%）表现显著落后。ACE智能体在结果正确性（1.0）、工具选择准确率（1.0）和执行成功率（1.0）上均达到最优，且用户干预次数和错误次数均为0。RAG智能体虽优于基础智能体，但仍有25%的任务失败。

**可能原因：** 基础智能体缺乏经验复用机制（复用率0%），导致重复错误率高（66.7%）。RAG智能体通过检索实现了经验复用（复用率41.7%），但无法主动积累新经验（新增率0%）。ACE智能体不仅复用经验（41.7%），还能主动添加新经验（新增率25%），从而在连续任务中逐步优化，避免了重复错误。

**论文可强调的结论：** 主动经验积累机制是提升GIS智能体在连续任务场景中鲁棒性的关键。ACE智能体通过“复用+新增”的双重学习策略，实现了零干预、零错误的稳定表现，证明了经验动态管理对复杂地理任务持续适应性的重要价值。
