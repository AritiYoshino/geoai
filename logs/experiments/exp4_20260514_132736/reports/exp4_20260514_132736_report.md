...
Run ID：exp4_20260514_132736

生成时间：2026-05-14T13:27:40
...
任务展开规模：0 个任务单元；本次 trace 数：300；成功 176，失败 124。
...
实验四任务集拆成 `exp4_workbook.json` 习题册和 `exp4_reference_answers.json` 参考答案。习题册定义 10 类通用能力任务模板并展开为 100 个 adaptation step，不局限于 GIS 任务；参考答案文件记录 collapse 判定阈值：高位 token 骤降到 35% 以下且 10 步滚动准确率下降至少 0.12。
...
| Collapse 事件数 | collapse_event_count | Exp4 中相邻 adaptation step 出现高位 token 骤降到低位，且滚动准确率同步下降的次数。 |

## 结果指标

| 组别 | average_runtime | average_turns | collapse_event_count | description | error_count | execution_success_rate | experience_add_rate | experience_reuse_rate | final_context_token_count | final_redundancy_rate | final_task_accuracy | knowledge_retention_rate | max_context_token_count | name | repeated_error_rate | result_correctness | task_success_rate | tool_selection_accuracy |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| monolithic_rewrite | 0.120 | 3.545 | 2 | 周期性把长上下文压缩为很短摘要，容易在压缩点丢失早期技能并触发 context collapse。 | 44 | 0.560 | 1.000 | 0.880 | 10980 | 0.763 | 0.780 | 1.000 | 13960 | Periodic Rewrite Baseline | 0.440 | 0.717 | 0.560 | 0.000 |
| append_only | 0.120 | 3.342 | 0 | 只追加任务记录和经验，不做结构化精炼；上下文持续膨胀，到达预算后被动截断。 | 78 | 0.220 | 1.000 | 0.900 | 15053 | 0.920 | 0.722 | 1.000 | 27652 | Append-only Baseline | 0.780 | 0.617 | 0.220 | 0.000 |
| ace_grow_and_refine | 0.120 | 2.848 | 0 | ACE 将任务反馈沉淀为结构化经验，并阶段性合并、去冗余和保留核心技能，使上下文平稳增长或温和精简。 | 2 | 0.980 | 1.000 | 0.900 | 12028 | 0.000 | 0.970 | 1.000 | 12028 | ACE Grow-and-Refine | 0.000 | 0.945 | 0.980 | 0.000 |
...
实验四中 ACE grow-and-refine 最终滚动准确率为 0.970，最终上下文 token 数为 12028，collapse 事件数为 0；可用于说明 ACE 在连续在线学习中能避免上下文坍塌。
