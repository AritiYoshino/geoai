...
Run ID：exp2_20260513_155123

生成时间：2026-05-13T16:01:23
...
实验二将任务按 batch 连续输入，对比 BASE 无记忆基线、RAG 静态经验检索和 ACE 连续学习三组，观察经验库积累后是否形成 ACE > RAG > BASE 的性能阶梯。

任务展开规模：18 个任务单元；本次 trace 数：54；成功 28，失败 26。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| base_agent | 0.164 | 3.924 | 14 | 0.222 | 0.000 | 0.000 | 0.000 | 0.722 | 0.646 | 0.222 | 1.000 | 14 |
| rag_agent | 0.164 | 3.454 | 9 | 0.500 | 0.000 | 0.278 | 0.000 | 0.500 | 0.790 | 0.500 | 1.000 | 9 |
| ace_agent | 0.164 | 3.136 | 3 | 0.833 | 0.222 | 0.500 | 0.667 | 0.111 | 0.953 | 0.833 | 1.000 | 0 |
...
实验二最终 batch 的成功率为 ACE=0.875、RAG=0.500、BASE=0.250；可结合经验复用率和新经验数量解释 ACE 连续学习相对静态检索与无记忆基线的优势。

基于连续任务学习实验（exp2）结果，主要发现如下：

**主要发现：** ACE智能体在任务成功率（0.833）、结果正确性（0.953）和执行成功率（0.833）上均显著优于RAG智能体（0.500/0.790/0.500）和基础智能体（0.222/0.646/0.222）。ACE智能体实现了零用户干预，而基础智能体需14次干预。在经验复用率上，ACE（0.500）高于RAG（0.278），且ACE是唯一能主动添加新经验（0.222）并具备修复能力（修复成功率0.667）的智能体。ACE的重复错误率（0.111）远低于基础智能体（0.722）和RAG（0.500）。

**可能原因：** ACE智能体通过主动积累和复用经验，在连续任务中逐步优化策略，其修复机制有效降低了重复错误，从而减少了对人工干预的依赖。RAG虽能复用经验但无法主动学习新经验，限制了性能提升。

**论文可强调的结论：** 经验主动积累与修复能力是提升GIS智能体在连续任务场景中自主性与鲁棒性的关键，ACE框架有效实现了从“被动响应”到“主动学习”的范式转变。
