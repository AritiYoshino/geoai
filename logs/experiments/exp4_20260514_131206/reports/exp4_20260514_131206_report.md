...
Run ID：exp4_20260514_131206

生成时间：2026-05-14T13:12:08
...
实验四采用简化的“习题册 + 参考答案”连续任务，比较整体重写、只追加、ACE 增长-精炼三种经验库更新策略，重点观察早期经验保留、冗余膨胀和最终回测表现。
...
实验四任务集拆成 `exp4_workbook.json` 习题册和 `exp4_reference_answers.json` 参考答案。习题册用 12 道连续任务覆盖早期经验学习、中期迁移复用、后期新增经验和最终早期经验回测；参考答案给出每题期望工具、输出类型、经验检索/新增约束和早期经验 id，用于计算成功率、知识保留率与冗余率。
...
实验四中 ACE grow-and-refine 最终回测准确率为 1.000，知识保留率为 1.000，可用于说明经验库增长与精炼策略在简化连续任务中的稳定性。
